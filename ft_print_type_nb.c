...
/*   Updated: 2021/03/01 09:07:12 by nvu              ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_print_nb_width(char *str, int len, t_flags *flags)
{
	if (flags->width > len)
	{
		...
		else
		{
			if (flags->other == 1)
				len += 1;
			if (!(str = ft_add_before(str, '0', len, flags->width - len)))
				return (NULL);
			if (flags->other == 1)
...
char	*ft_print_nb(char *str, int len, t_flags *flags)
{
	...
	{
		if (!(str = ft_add_before(str, '0', len, flags->precision - len)))
			return (NULL);
	}
	if (flags->other == 1 && (flags->width <= len || (flags->minus == 1
			|| ((flags->zero == 0) || (flags->zero == 1
			&& flags->precision != -2)))))
